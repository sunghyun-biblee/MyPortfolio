...
export const ClassHubData = {
  ...
  troubleShooting: [
    ...
    },
    {
      ...
      serach:
        "**[중간과정]** 회의 결과, 배포하기 전에는 `Access-Control-Allow-Origin`을 localhost로 설정하여 CORS 오류가 발생하지 않았습니다. 그러나 배포 후에도 동일한 설정을 추가 적용했음에도 불구하고 접근이 허용되지 않았습니다. 또한 스프링 시큐리티가 추가되면서 문제 발생 지점을 찾기 위해서는 `많은 시간이 필요할 것으로 예상`되었습니다.",
      Resolution:
