  ...
  skills: [
    {
      name: "React",
      description:
        "React는 컴포넌트 기반으로 동작하며, Virtual DOM을 통해 웹 애플리케이션의 성능을 최적화합니다. 컴포넌트 재사용성을 높이고, useState를 통해 상태를 관리하며, useEffect를 사용하여 API 호출을 최소화하는 과정을 거치면서 React Hook의 기본을 익힐 수 있었습니다.",
    },
    {
      name: "framer-motion",
      description:
    ...
    },
    {
      name: "Firebase",
      description:
        "Firebase는 유저 인증, 데이터베이스, API 등 반복적으로 사용되는 기능을 간편하게 제공합니다. 프로젝트 팀원이 2명으로 줄어들면서, 다른 부분에 더 많은 시간을 투자하기 위해 Firebase의 Authentication 및 OAuth 기능을 활용하여 간단하게 SNS 로그인을 구현하였습니다. 이를 통해 유저 관련 CRUD 과정을 직접 구현하지 않아도 되어 많은 시간을 절약할 수 있었습니다",
    },
  ],
  summary:
    "'Trip_Fixied'는 사용자의 여행 일정에 맞춰 관광지, 축제 정보, 숙소 등 다양한 선택지를 제공하여\n\n 여행 일정을 계획하고 비교하는 데 도움을 주는 서비스입니다.",
  background:
    "많은 사람이 여행을 계획할 때 발생할 수 있는 선택 고민과 정보 부족 문제를 겪습니다.\n 여행지를 정하고 나서도 어떤 활동을 할지 모르거나, 해당 지역의 관광 정보를 찾기 어려운 경우가 많습니다.\n 저 또한 여행을 계획할 때 이러한 불편함을 겪어봤었고, 누군가가 추천해주는 시스템이 있다면 어떨까라는\n 고민과 함께 개발하게 되었습니다. ",
  mainFuntion: [
  ...
  myActivities: [
    {
      ...
      description:
        "인터랙티브한 웹 디자인을 좋아하는 저는 페이지 전환 시 애니메이션을 적용해보자 라는 생각에 Framer Motion을 활용하여, 컴포넌트가 마운트 및 언마운트될 때 애니메이션을 추가했습니다.",
    },
    {
    ...
    {
      title: "OpenWeatherAPI를 통해 날씨정보 제공",
      description:
  ...
  troubleShooting: [
    {
      ...
      trouble:
        "**[문제점]** 여행지의 정보를 무한스크롤로 적용하여 리스트로 보여주게되는데 뷰포트가 바닥을 감지하였을때 API를 호출하다보니 일일 API 호출 트래픽이 짧은 시간에 초과하는 상황",
      serach: null,
      Resolution:
        "**[해결방안]** 첫 데이터 호출 시 모든 데이터를 가져와 이중 배열로 나누어 저장하고, 스크롤 이벤트 발생 시 index를 증가시키고 ,증가된 index를 참조하여 데이터를 출력하도록 하여 API 호출 수를 약 90% 가량 감소시켰습니다",
      myThink: null,
    },
  ],
  ProjectIMG: [],
