export const ClassHubData = {
  ...
  skills: [
    ...
    },
    {
      name: "React-Query",
      description:
        "React Query는 React 애플리케이션에서 데이터 가져오기, 관리 및 캐싱을 사용하기 쉽게 해주는 라이브러리입니다. 서버로부터 데이터를 비동기적으로 가져오고 Mutation을 사용하여 데이터를 낙관적 업데이트하여 사용자 경험을 향상 시킵니다. API 요청을 처리하고 캐싱하여 성능을 최적화할 수 있습니다.",
    },
    {
      name: "Typescript",
      description:
  ...
  summary:
    "클래스허브는 언제 어디서나 접근 가능한 온라인 학습 플랫폼으로 \n다양한 프로그래밍 주제와 전문가 강의를 통해 학생들에게 편리하고 효율적인 학습 경험을 제공합니다.",
  background:
    "개발자로 성장하는 과정에서 독학으로 여러 학습 시스템을 활용한 경험이 많았습니다. 특히 인프런, 유데미 같은 학습 플랫폼에서 제공되는 영상 이어보기와 학습 현황 추적 기능을 직접 구현해보고 싶었습니다. 또한 학습자들과 소통할 수있는 기능과 특히 독학자를 위한 스터디 모집 기능을 구현해보면 좋을 것 같다는 생각이 들어서, 효율적인 학습 환경을 구현해보고자 프로젝트를 기획하게 되었습니다. ",
  mainFuntion: [
  ...
  troubleShooting: [
    ...
    },
    {
      ...
      trouble:
        "로컬에서 작업할 때는 CORS에러가 발생하지 않았으나, 스프링 시큐리티 및 배포작업을 마치고난 뒤 로컬에서도 CORS에러가 발생",
      serach:
        "회의 결과, 배포하기 전에는 Access-Control-Allow-Origin을 localhost로 설정하여 CORS 오류가 발생하지 않았습니다. 그러나 배포 후에도 동일한 설정을 추가 적용했음에도 불구하고 접근이 허용되지 않았습니다. 또한 스프링 시큐리티가 추가되면서 문제 발생 지점을 찾기 위해서는 많은 시간이 필요할 것으로 예상되었습니다.",
      Resolution:
        "CORS 오류로 인해 모든 API 호출이 차단되어 아무 작업도 할 수 없는 상황을 해결하기 위해 http-proxy-middleware 라이브러리를 사용하여 프록시를 설정하여 CORS 문제를 해결했습니다",
      myThink:
        "이번 프로젝트에서 API에 매우 의존적이었기 때문에 CORS 오류가 발생하면 아무 작업도 할 수 없다는 점을 실감했습니다. 이런 상황을 해결하기 위해 Mock Service Worker의 필요성을 느꼈고, 이를 대비하여 미리 Mock 데이터를 활용할 수 있는 방법에 대해 공부해야겠다고 결심했습니다.",
    },
  ],
  ProjectIMG: [],
};
